...
from it_eli_mcp.coverage import build_coverage
from it_eli_mcp.server import it_coverage


def _coverage():
    fn = getattr(it_coverage, "fn", it_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("IT_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("it_coverage" in line for line in lines), (
        "an audit entry was written but it does not name it_coverage"
    )
