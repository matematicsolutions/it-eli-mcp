...
SOURCE = 'normattiva.it, giustizia-amministrativa.it and Cassazione'
...
_FAMILIES: list[dict] = [{'name': 'Legislation (Normattiva)', 'tool': 'it_list_codes / it_resolve / it_get_act / it_get_text'}, {'name': 'Administrative case law', 'tool': 'it_ga_search / it_ga_get_decision'}, {'name': 'Cassazione case law', 'tool': 'it_cassazione_search / it_cassazione_get'}]

_GAPS: list[dict] = [{'id': 'IT-001', 'family': 'Legislation (Normattiva)', 'missing': 'Regional legislation is out of scope - Normattiva carries state law.', 'fallback': "Use the region's own bollettino ufficiale."}, {'id': 'IT-002', 'family': 'Legislation (Normattiva)', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}, {'id': 'IT-003', 'family': 'Administrative case law', 'missing': 'Coverage follows what each court portal publishes; this is not an exhaustive record of Italian case law.', 'fallback': "Cross-check on the court's own portal for exhaustive research."}]
