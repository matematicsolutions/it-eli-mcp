...
Every call to every MCP tool writes one JSON line to:
    ...
    ~/.matematic/audit/it-eli-mcp.jsonl

(or to the directory given by ``IT_ELI_AUDIT_DIR``).
...
(CONSTITUTION.md art. 2). Inherited from the eu-legal-mcp production line.
"""
...
_AUDIT_FILENAME = "it-eli-mcp.jsonl"


def _resolve_audit_dir() -> Path:
    env = os.environ.get("IT_ELI_AUDIT_DIR")
    if env:
...
class AuditLogger:
    ...
    def log(
        ...
        *,
        ...
        # If this fails, it fails (CONSTITUTION art. 2).
